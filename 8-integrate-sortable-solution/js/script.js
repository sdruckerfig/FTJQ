(function($){ 
	...
	$(document).ready(function() {
		...
		// $("#peopleTable tbody").sortable();
		$("#peopleTable tbody").sortable({handle:"td.handle", helper: fixHelper});

		$("#peopleTable tbody").on("sortstop", function(event){
			$("#peopleTable").trigger("selectableselecting");
		});

		function fixHelper(event, ui) {
			var $originalRow = ui.children();
			var $helperRow = ui.clone();
			$helperRow.children().each(function(index, item){
				$(this).width($originalRow.eq(index).width());
			});
			return $helperRow;
		}
